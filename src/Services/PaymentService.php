...
class PaymentService
{
    ...
    public function getInvoicePrepaymentComments($requestData)
    {
        ...
        if(isset($paymentReference))
        {
            ...
            $i = 1;
            $countReference  = $references[1] + $references[1];
            ...
            foreach ($references as $key => $value)
            {
                if ($references[$key] == 1)
                {
                    ...
                    $invoiceComments .= ($key == 1) ? ('BNR-' . $productId . '-' . $requestData['order_no']) : ($key == 2 ? 'TID '. $requestData['tid'] : '');
                }
            }
    ...
    public function getRequestParameters(Basket $basket, $paymentKey)
    {
        ...
        if(in_array($paymentRequestData['key'], ['33', '34', '49', '50', '69', '78']))
        {
        ...
        $referrerId = $this->paymentHelper->getNovalnetConfig('novalnet_referrer_id');
