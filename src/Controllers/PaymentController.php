...
class PaymentController extends Controller
{
    ...
    public function processPayment()
    {
        ...
        if(!empty($requestData['paymentKey']) && in_array($requestData['paymentKey'], ['NOVALNET_CC', 'NOVALNET_INVOICE', 'NOVALNET_SEPA']) && (!empty($requestData['nn_pan_hash']) || !empty($requestData['nn_invoice_birthday']) || !empty($requestData['nn_sepa_birthday'])))
        ...
        if($requestData['paymentKey'] == 'NOVALNET_CC') {
            ...
            if($this->config->get('Novalnet.novalnet_cc_3d') == 'true' || $this->config->get('Novalnet.novalnet_cc_3d_fraudcheck') == 'true' )
            {
                ...
                $this->getLogger(__METHOD__)->error('test',$serverRequestData['data'] );
